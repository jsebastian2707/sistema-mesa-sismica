...
import time
import state
from logic import (find_serial_ports, connect_serial, disconnect_serial, 
...
def main():
    dpg.create_context()
    create_gui()       
    dpg.create_viewport(title='control mesa sismica', width=1200, height=1000, x_pos=0, y_pos=0)
    dpg.setup_dearpygui()
    dpg.show_viewport()

    # Start your Serial Thread
    threading.Thread(target=read_serial_thread, daemon=True).start()

    # --- THE MANUAL LOOP ---
    while dpg.is_dearpygui_running():
        
        # This function updates your plots/text from the serial data
        # update_gui_callbacks()
        
        # Render the frame
        dpg.render_dearpygui_frame()

    # cuando el bucle principal se termina se corta el serial y la flag the running se baja
    disconnect_serial()
    state.running = False
    dpg.destroy_context()
